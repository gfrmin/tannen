...
>
> It carries the driver changes that were **confirmed live on 2026-08-31** and cannot be
> invalidated by anything the red team finds. Two things are still missing, and the sitting
> should not be run until they land:
>
> 1. **The M2 boundary red team has not been run.** BRIEF §9.1 point 5 mandates one at every
>    boundary; M0's was D0059, M1's was D0104, and `grep -r RT-M2` currently returns nothing.
>    Its findings are what step 5 installs. Until it runs, step 5 installs *nothing new* — and
>    a sitting that installs no fixture because the red team found nothing is a different
>    state from one that installs none because nobody looked. See **Red-team findings** below.
> 2. **The queue's ready text is not drafted** — D0131 items 2/3/4/6/8, D0123's binding
>    upgrades, and D0138's two. See **Still to draft** below.
...
*(Nothing here yet — the M2 boundary red team has not been run. Its mandate is in the plan for
this work; its outputs belong at `docs/redteam/2026-08-31-m2-boundary.md`, a Tier-A record, and
fixture candidates under `docs/redteam/fixture-candidates/`. When it lands, add each fixture name
to step 5's `FIXTURES` list at the marked slot, add a `readme_row_absent` block per fixture, and
write the table rows into a `poison-readme-rows.md` beside this file.)*
...
- **D0131 item 2** — D0117's evidence-record quantity. `governance/schemas/evidence-record.schema.json`
  ...
  cannot touch it.
- **D0131 item 3** — `scripts/check_laws.py` and `governance/laws.yaml` into the custody set. A
  guard that can retire a frozen law, and the data it reads, are today ordinary builder-editable
  files. The red team's probe 3 will have priced exactly what that is worth.
